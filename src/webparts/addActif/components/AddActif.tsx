...
import {Checkbox, DefaultButton, Dialog, DialogFooter, DialogType, Dropdown, IDropdownOption, IDropdownStyles, Label, Panel, PrimaryButton, Stack, TextField } from 'office-ui-fabric-react';
...
function Add (props:IAddActifProps){
    ...
    let [form, setForm] = React.useState({Dénomination :"", Ville:0, type_de_bien:"",Nombre_dactifs:0, type_de_commercialisation:[],Valorisation_location:0,Valorisation_Vente:0, Statut_dactivité:"",Commercialisation_en_valeur:0,Commercialisation_en_nombre:0,Date_Mandat:""});
    ...
    async function _onSubmit(){
      ...
      await sp.web.lists.getByTitle("Listing").items.add({
        ...
        setForm({
          ...form, Dénomination :"", Ville:0, type_de_bien:"",Nombre_dactifs:0, type_de_commercialisation:[],Valorisation_location:0,Valorisation_Vente:0, Statut_dactivité:"",Commercialisation_en_valeur:0,Commercialisation_en_nombre:0,Date_Mandat:""
        });
        await createFolder("Photos%20Gnrales", form.Dénomination);
        await createFolder("Documents Générales/"+form.Dénomination, "Documents "+form.Dénomination);
        await createFolder("Documents Générales/"+form.Dénomination, "Photos "+form.Dénomination);
        setAlert(true);
      }
)
  }
    return(
      ...
      <Panel isOpen={isOpen} onDismiss={()=> setIsOpen(false)} headerText="Information d'Actif" closeButtonAriaLabel="Close">
        ...
        <TextField label="Dénomination" placeholder="Entrer la Dénomination de l'actif" onChange={(e) => setForm({...form, Dénomination:(e.target as HTMLInputElement).value}) }/>
